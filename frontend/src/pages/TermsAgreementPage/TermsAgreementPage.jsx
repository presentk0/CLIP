...
export default function TermsAgreementPage() {
  ...
  const openExternalLink = (url) => {
    ...
    // typeof chrome !== 'undefined': chrome 객체가 존재하는가?
    // if (chrome !== undefined) { 는 에러나서 typeof를 사용
    // typeof는 존재하지 않는 변수에 접근할 수 있는 유일한 방법
    // 크롬 확장프로그램: 'object'
    // 일반 웹: 'undefined'

    // chrome.tabs: chrome.tabs API 사용 가능한가?
    // 확장프로그램(권한 있음): 객체
    // 확장프로그램(권한 없음): undefined
    // 일반 웹: 접근 자체 불가
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      // 크롬 확장프로그램 환경이면 새 탭 열기
      chrome.tabs.create({ url });
    } else {
      // 일반 웹 환경이면 window.open으로 새 탭 열기
      // '_blank' 는 새 탭
      // 'noopener,noreferrer' 는 보안 옵션
