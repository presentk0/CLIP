...
const DIFFICULTY_OPTIONS = [
  { value: 'EASIER',  label: '지금 보다 가볍게 갈게요', offset: -2 },
  { value: 'RELAXED', label: '여유롭게 갈게요',          offset: -1 },
  { value: 'CURRENT', label: '지금이 좋아요',            offset:  0 },
...
const SAFE_ERRORS = {
  INVALID_PREFERENCE: '올바르지 않은 설정입니다',
...
export default function DifficultySelectPage() {
  ...
  useEffect(() => {
    ...
    if (!onboardingData.learningGoal) {
  ...
  const handleSelect = (value, disabled) => {
    if (disabled) return;
    if (!VALID_DIFFICULTY.includes(value)) return;
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await apiFetch('/preferences/onboarding', {
        method: 'POST',
        body: JSON.stringify({
          learningGoal,
          difficultyLevel,
      ...
      if (!res.success) {
        const err = new Error(res.error?.message || '저장 실패');
      ...
      setError(handleApiError(err, '온보딩 저장', SAFE_ERRORS) || '저장에 실패했습니다');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className={styles.level4}>
            ...
            <div className={styles.level8}>
              <div className={styles.level9}>
                <div className={styles.level10}>
                  <p className={styles.level11}>현재 학습 난이도를 선택해주세요</p>
                  {error && <div role="alert">{error}</div>}
                </div>

                <div className={styles.level12}>
                  <p className={styles.level13}>나중에 언제든 다시 바꿀 수 있어요</p>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className={styles['difficulty-card8']}>
      {DIFFICULTY_OPTIONS.map(({ value, label, offset }) => {
        ...
        return (
          ...
          >
            <p className={styles.level24}>{isLoading ? '저장 중...' : '확인'}</p>
          </button>
        </div>
      </div>
