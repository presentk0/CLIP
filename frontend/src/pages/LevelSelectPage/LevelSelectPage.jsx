...
const VALID_VALUES = ABSOLUTE_OPTIONS.map((o) => o.value);

export default function LevelSelectPage() {
  const navigate = useNavigate();
  const { needsOnboarding } = useAuth();
  const { onboardingData, setAbsoluteLevel } = useOnboarding();
  ...
  const selected = onboardingData.absoluteLevel;

  const handleSelect = (value) => {
    if (!VALID_VALUES.includes(value)) return;
    setAbsoluteLevel(value);
  };
  ...
  const handleBack = () => navigate('/onboarding/terms', { replace: true });
    ...
    <div className={styles.level}>
      <div className={styles.level2}>
            ...
            <div className={styles.level8}>
              <div className={styles.level9}>
                <div className={styles.level10}>
                  <p className={styles.level11}>현재 영어 실력을 선택해주세요</p>
                </div>
        ...
        <div className={styles.level14}>
          <div className={styles.level15}>
            <div className={styles.level16}>
              <div 
              role="radiogroup"
              aria-label="영어 실력 선택"
              className={styles.level17}>
          ...
          disabled={!selected}
          >
            <p className={styles.level24}>확인</p>
          </button>
        </div>
      </div>
    </div>
